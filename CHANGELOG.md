...
- **Scrubber filmstrip after import** — When a scan adds files, Skagway starts baking the in-player scrubber strip for the first clip in the current sort, at the player size it will actually open. The strip stays blank until that bake is ready; Play should find it already on disk.
