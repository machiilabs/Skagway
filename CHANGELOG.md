...
- **Repair Links thumbnail UI refresh** — After migrate, immediately recompute filtered rows and bump `filmstripRefreshId` + `filteredVideosVersion` + a fresh `thumbnailPath#version` so grid/list cards reload without changing filters. Memory cache is cleared and re-warmed from migrated disk files.
- **Fix: Repair Links thumbs wrong until relaunch** — `migrateCacheKeys` no longer warms `NSCache` before detail files finish moving (that left auto-frames in memory while disk was already correct). Cards clear `@State`, include `thumbnailReloadId` in `.task(id:)`, and remount with `filmstripRefreshId` in `.id`.
