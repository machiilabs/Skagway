...
- **Fix: Hide/Show Inspector (⌘I) stall** — Toggling the Inspector only changes split width. Grid/Storyboard no longer remount on the column-count change (single adaptive `GridItem` instead of `LazyVGrid(columns: count:)`), card `.task` keeps resident Storyboard collages instead of clearing and re-decoding, and scroll-pin restore no longer 1pt-retiles every visible card.
