...
- **Fix: Hide/Show Inspector (⌘I) stall** — Toggling the Inspector only changes split width. Column count is computed from inspector-compensated width so ⌘I does not reflow the LazyVGrid cache accumulated from scrolling. Off-screen Storyboard `.task`s no longer bake/AV-generate (visible-window gate + cancellable inflight). Pin restore no longer bumps an `@Observable` token or `layoutSubtreeIfNeeded` the whole document. Eager context menus build only for hovered/focused/selected cards.
