...
- **Repair Links open UX (large libraries)** — Sheet opens immediately on Repair Links… (no click freeze). Step 1 shows “Building folder list…” with a horizontal progress bar while the catalog builds off the main actor; Continue stays disabled until ready. Pre-selected old folder scrolls into view; folder list sorted A–Z by full path.
